...
// Gap (AI): No '/curriculum
// No /curriculum-gap-detect (identify missing topics from student performance)
import React, { useState } from 'react';

export default function GapNoCurriculumPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-no-curriculum/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>No '/curriculum</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>No /curriculum-gap-detect (identify missing topics from student performance)</p>
