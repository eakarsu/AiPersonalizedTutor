...
// Gap Feature: No payment/billing for parent subscriptions
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AiPersonalizedTutor / gap-no-payment-billing-for-parent-subscriptions',
    },
    body: JSON.stringify({
...
router.post('/run', async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'No payment/billing for parent subscriptions'.\nProject: AiPersonalizedTutor.\nContext: No payment/billing for parent subscriptions\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['no-payment-billing-for-parent-subscriptions', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'no-payment-billing-for-parent-subscriptions', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
