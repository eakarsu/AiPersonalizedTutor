...
// AI analyzes student learning style (visual, kinesthetic, reading); recommends resources
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AiPersonalizedTutor / cf-content-recommendation',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_content_recommendation`;
(async () => {
  try {
    await pool.query(`
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant helping with the feature: 'Content recommendation'.
Context: AI analyzes student learning style (visual, kinesthetic, reading); recommends resources
...
If input is insufficient, populate sensible defaults and note assumptions in summary.`;
    ...
    console.error('cf-content-recommendation/run error:', err);
