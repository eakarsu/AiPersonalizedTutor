...
// Avatar/chatbot provides hints and explanations when student is stuck
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AiPersonalizedTutor / cf-automated-tutoring',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_automated_tutoring`;
(async () => {
  try {
    await pool.query(`
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant helping with the feature: 'Automated tutoring'.
Context: Avatar/chatbot provides hints and explanations when student is stuck
...
If input is insufficient, populate sensible defaults and note assumptions in summary.`;
    ...
    console.error('cf-automated-tutoring/run error:', err);
