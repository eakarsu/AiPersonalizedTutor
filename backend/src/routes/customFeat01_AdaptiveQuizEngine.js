...
// Difficulty adjusts based on student performance; AI suggests next topics to study based on knowledge gaps
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AiPersonalizedTutor / cf-adaptive-quiz-engine',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_adaptive_quiz_engine`;
(async () => {
  try {
    await pool.query(`
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant helping with the feature: 'Adaptive quiz engine'.
Context: Difficulty adjusts based on student performance; AI suggests next topics to study based on knowledge gaps
...
If input is insufficient, populate sensible defaults and note assumptions in summary.`;
    ...
    console.error('cf-adaptive-quiz-engine/run error:', err);
