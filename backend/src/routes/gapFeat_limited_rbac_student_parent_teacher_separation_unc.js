...
// Gap Feature: Limited RBAC (student/parent/teacher separation unclear)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AiPersonalizedTutor / gap-limited-rbac-student-parent-teacher-separation-unc',
    },
    body: JSON.stringify({
...
router.post('/run', async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Limited RBAC (student/parent/teacher separation unclear)'.\nProject: AiPersonalizedTutor.\nContext: Limited RBAC (student/parent/teacher separation unclear)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['limited-rbac-student-parent-teacher-separation-unc', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'limited-rbac-student-parent-teacher-separation-unc', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    ...
    const r = await _pool.query(
      ...
      ['limited-rbac-student-parent-teacher-separation-unc']
    );
