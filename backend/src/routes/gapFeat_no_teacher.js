...
// Gap Feature: No \'/teacher
// No /teacher-assistant-ai (help teachers grade/respond to many students)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AiPersonalizedTutor / gap-no-teacher',
    },
    body: JSON.stringify({
...
router.post('/run', async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'No \'/teacher'.\nProject: AiPersonalizedTutor.\nContext: No /teacher-assistant-ai (help teachers grade/respond to many students)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['no-teacher', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'no-teacher', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
