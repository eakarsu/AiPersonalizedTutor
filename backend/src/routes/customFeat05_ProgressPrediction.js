...
// ML forecasts likelihood of passing exams; recommends study intensity
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AiPersonalizedTutor / cf-progress-prediction',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_progress_prediction`;
(async () => {
  try {
    await pool.query(`
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant helping with the feature: 'Progress prediction'.
Context: ML forecasts likelihood of passing exams; recommends study intensity
...
If input is insufficient, populate sensible defaults and note assumptions in summary.`;
    ...
    console.error('cf-progress-prediction/run error:', err);
